...
router = APIRouter(prefix="/admin/period", tags=["Admin period"])
...
async def create_period(period_in: PeriodCreate, db: AsyncSession = Depends(get_db),
    ...
    try:
        ...
        await crud_audit_log.create_audit_log(db, AuditLogCreate(
            user_id=user.id,
            action=f"/admin/period",
            model="post",
            model_id=user.username,
            details=f"Create period {period.Name}",
...
async def get_periods(db: AsyncSession = Depends(get_db), user: UserOut = Depends(get_current_admin)):
    try:
        if not user.is_admin:
            raise HTTPException(status_code=403, detail="Permission denied")
...
async def get_all_active_periods(
...
async def get_period(period_id: str, db: AsyncSession = Depends(get_db), user: UserOut = Depends(get_current_admin)):
    try:
        if not user.is_admin:
            raise HTTPException(status_code=403, detail="Permission denied")
...
async def update_period(period_id: str, period_in: PeriodUpdate, db: AsyncSession = Depends(get_db), user: UserOut = Depends(get_current_admin)):
    try:
        if not user.is_admin:
            raise HTTPException(status_code=403, detail="Permission denied")
        result = await db.execute(select(Period).where(Period.ID == period_id))
        ...
        await crud_audit_log.create_audit_log(db, AuditLogCreate(
            user_id=user.id,
            action=f"/admin/period/{period_id}",
            model="put",
            model_id=user.username,
            details=f"Create period {period_in.Name}",
            timestamp=now.replace(tzinfo=None)
        ))
        return period
    except Exception as e:
...
async def delete_period(period_id: str, db: AsyncSession = Depends(get_db), user: UserOut = Depends(get_current_admin)):
    try:
        if not user.is_admin:
            raise HTTPException(status_code=403, detail="Permission denied")
