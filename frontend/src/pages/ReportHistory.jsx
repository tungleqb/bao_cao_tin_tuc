...
const ReportHistory = () => {
  ...
  const handleDownload = async (periodId) => {
    try {
      const response = await axios.get(`/report/download/me/${periodId}`, {
        responseType: "blob",
      });

      const contentDisposition = response.headers["content-disposition"];
      let filename = "baocao.docx";
      ...
      if (error.response?.status === 404) {
        alert(error.response.data.detail || "Không có báo cáo hoặc file không còn tồn tại.");
      } else {
        ...
        <table className="table-auto w-full text-sm text-left border-collapse">
          <thead>
            <tr className="bg-gray-100 text-sm font-semibold">
              ...
              <th className="px-2 py-1">Tương tác</th>
            </tr>
              ...
              <tr key={r.ID} className="border-t">
                ...
                <td className="px-2 py-1 text-center">
                  <button
                    ...
                    onClick={() => handleDownload(r.PeriodID)}
                  >
                    Tải về
                  </button>
                </td>
